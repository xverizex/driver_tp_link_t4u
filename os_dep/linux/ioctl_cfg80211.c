...
#endif
#if 0
	static struct wiphy_vendor_command ven[1];
	ven[0].policy = VENDOR_CMD_RAW_DATA;
	ven[0].doit = 1;
	ven[0].dumpit = 1;
	wiphy->vendor_commands = &ven[0];
	wiphy->n_vendor_commands = 1;
#endif
	int ret = wiphy_register(wiphy);

	return ret; //wiphy_register(wiphy);
}
